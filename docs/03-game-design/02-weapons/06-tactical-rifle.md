...
**Used by:** Nomad  **Slot:** primary

## Summary

Bridges assault and sniper with burst/semi-auto fire, better accuracy than the AR, lower fire rate.
...
| Range | 65 |
| Headshot Mult | 2.2 |
| Movement Penalty | 0.95 |
| Ads Ms | 200 |

Values above are documentation snapshots of `packages/config/weapons.json["tactical-rifle"]` — treat
...
- **Client-side damage application** — The Tactical Rifle's damage must be computed and applied server-side from a validated hit; the client only plays feedback (hitmarker, sound) in response to a server event.
