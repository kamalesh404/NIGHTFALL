...
**Used by:** Falcon  **Slot:** primary

## Summary

Three-round bursts, high accuracy, moderate recoil, rewards trigger discipline.
...
|---|---|
| Damage | 26 |
| Fire Rate | 3x14 burst |
| Reload | 2.1 |
| Magazine | 24 |
| Accuracy | 0.6 |
| Range | 55 |
| Headshot Mult | 2.2 |
| Movement Penalty | 0.97 |
| Ads Ms | 170 |

Values above are documentation snapshots of `packages/config/weapons.json["burst-rifle"]` — treat
...
- **Client-side damage application** — The Burst Rifle's damage must be computed and applied server-side from a validated hit; the client only plays feedback (hitmarker, sound) in response to a server event.
