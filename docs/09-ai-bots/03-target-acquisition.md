...
A bot considers an enemy a valid target if within its vision cone (configurable angle/range) and a
raycast confirms unobstructed line of sight. Multiple visible targets are scored by a priority
function (distance, current bot health, target health) rather than always engaging the nearest.
