...
Neon PostgreSQL automated backups/point-in-time recovery per its managed offering. Rollback
procedure for a bad deploy: redeploy the previous known-good build via the CI/CD pipeline's build
history (never hand-patch production). Incident response follows the escalation path in
13-security/07-incident-response.md for anything security-relevant.
