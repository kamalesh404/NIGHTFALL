...
Client deploys to Vercel (recommended) or Cloudflare Pages, building `apps/client` via the
monorepo-aware build command. Automatic builds trigger on push to `main` (staging) with preview
deployments per PR; production is a manual promotion.
