...
Client and server both report to Sentry with release tracking tied to the deployed commit SHA, so
an error can be traced back to the exact build. PII scrubbing rules match 13-security/06-data-privacy.md.
