...
`docker-compose up` brings up Postgres + Redis + the client dev server + the game server together.
Environment variables are documented in `.env.example` (see 14-development/08-environment-variables.md)
and required before first run (`DATABASE_URL`, `JWT_SECRET`, etc.).
