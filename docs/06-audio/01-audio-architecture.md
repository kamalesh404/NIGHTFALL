...
One `AudioContext` for the app lifetime (resumed on first user gesture per browser autoplay
policy). Graph: source → `PannerNode` (spatial sources only) → category `GainNode` (weapons /
footsteps / ambient / UI) → master `GainNode` → destination. Category and master gain nodes are
what Settings volume sliders control (07-ui-ux/10-settings-menu.md).
