...
GPU instancing (`THREE.InstancedMesh`) renders repeated objects (trees, rocks, containers) in a
single draw call. LOD (`THREE.LOD`) switches to lower-polygon versions of distant objects at
configured distance thresholds per quality preset — Low/Medium use more aggressive LOD switching
than High/Ultra to hit their respective draw-distance budgets.
