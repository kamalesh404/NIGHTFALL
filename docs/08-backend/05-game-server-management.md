...
`LobbyManager` owns the lifecycle of `MatchRoom` instances: creation on demand, player slot
tracking, bot fill/eviction (04-networking/10-bot-filling.md), and teardown once a match's post-vote
transition completes or the room empties. Rooms are garbage-collected (removed from the active
registry) once empty for a short grace period, not kept alive indefinitely.
