...
Three layers: unit tests (Vitest) for pure logic (damage calculation, XP curve, recoil pattern
application), integration tests (Vitest) for API routes, database operations, and Socket.IO message
handling, and manual test plans for gameplay feel that automated tests can't meaningfully assert on
(see 04-manual-testing.md). Playwright E2E is optional and scoped to menus/auth flows only — not
in-match gameplay, which is impractical to drive through a real browser reliably.
