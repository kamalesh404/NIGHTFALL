...
Category gain nodes (weapons/footsteps/ambient/UI) allow independent volume control and simple
priority ducking — e.g., kill confirmation and medal announcement sounds briefly duck the ambient
and footstep categories so they're always audible. Ducking is implemented as a temporary gain
reduction with a short attack/release envelope, not a hard mute.
