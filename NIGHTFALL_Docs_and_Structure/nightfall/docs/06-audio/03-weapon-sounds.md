...
Each weapon class has a unique gunshot sound (distinct timbre, length, decay) and a unique reload
sound, generated procedurally via Web Audio synthesis (oscillators + noise + envelope shaping) or
short pre-rendered buffers — either way, not fetched licensed audio files (see 08-procedural-sound-generation.md).
