...
cd nightfall
pnpm install
cp .env.example .env        # fill in required values, see 08-environment-variables.md
docker compose up -d        # Postgres + Redis
pnpm --filter server prisma migrate dev
pnpm dev                    # runs client (Vite) + server (tsx watch) together
```
Client dev server defaults to `http://localhost:5173`; game server to `http://localhost:3000`.
