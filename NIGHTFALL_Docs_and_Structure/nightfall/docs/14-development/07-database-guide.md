...
`pnpm --filter server prisma studio` opens a local DB browser/editor. Migrations: author schema
changes in `schema.prisma`, run `prisma migrate dev --name <description>` locally to generate and
apply a migration, commit the generated migration files. Seeding: `prisma/seed.ts` populates a
minimal dev dataset (a few test accounts, no production data ever seeded).
