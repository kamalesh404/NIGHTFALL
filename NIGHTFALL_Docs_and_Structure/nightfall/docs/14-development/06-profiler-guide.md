...
Chrome DevTools Performance tab for frame-time breakdown (script vs. render vs. GPU). Three.js
inspector browser extension for scene-graph/draw-call/geometry inspection. Chrome Memory tab for
heap snapshots when investigating the memory budget (02-tech-stack/10-performance-budgets.md). Use
these to confirm a bottleneck before optimizing — see 05-rendering/12-performance-profiling.md.
