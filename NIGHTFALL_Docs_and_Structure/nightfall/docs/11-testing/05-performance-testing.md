...
FPS benchmarks run against the budgets in 02-tech-stack/10-performance-budgets.md using a scripted
8-bot stress match per map/mode combination. Memory profiling checks for leaks across a sustained
multi-match session (heap should return to baseline between matches, not grow unbounded). Load
tests simulate concurrent `MatchRoom`s to validate server tick stability under realistic load.
