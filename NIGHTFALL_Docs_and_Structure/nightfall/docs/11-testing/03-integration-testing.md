...
API tests hit Express routes against a test database (migrated fresh per test run). Socket tests
spin up a real `MatchRoom` instance and drive it with a mock Socket.IO client to verify tick
behavior, mode rule outcomes, and message contracts end-to-end (not just unit-level mode logic).
