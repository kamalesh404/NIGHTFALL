...
Gameplay feel (movement responsiveness, weapon TTK balance, bot difficulty feel) is verified via
structured manual test passes per phase milestone, using a lightweight bug report template
(repro steps, expected vs actual, build/commit). This is not a substitute for automated tests on
logic that can be asserted programmatically — manual testing covers subjective feel only.
