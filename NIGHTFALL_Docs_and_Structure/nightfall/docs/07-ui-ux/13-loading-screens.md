...
Short by design — there are no external assets to download, so loading screens mainly cover
procedural geometry/texture generation and connection setup. Shows a gameplay tip, generation
progress, and target map info (name, theme, mode).
