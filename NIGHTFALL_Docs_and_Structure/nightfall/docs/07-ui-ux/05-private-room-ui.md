...
Create flow generates and displays a shareable room code with a one-click copy button. Join flow is
a code-entry field with client-side format validation (length/charset) before the join request.
Both flows show a live player list once inside the lobby.
