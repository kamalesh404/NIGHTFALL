...
All traffic over WSS/HTTPS in production (no plaintext WS/HTTP). CORS locked to known frontend
origins. Basic rate limiting (08-backend/10-rate-limiting.md) mitigates casual DoS/abuse; full DDoS
protection is delegated to the hosting platform's edge network rather than reimplemented
server-side.
