...
bcrypt cost factor tuned to a safe modern default (≥10 rounds, revisited as hardware improves). JWT
secret stored only in server environment variables, rotated on suspected compromise. Cookies:
HttpOnly, Secure, SameSite=Lax (or Strict where it doesn't break the intended cross-origin flows).
